...
{
    // return if URI is not valid or is no qednet: URI
    if(!uri.isValid() || uri.scheme() != QString("qednet"))
        return false;
...
{
    // Convert qednet:// to qednet:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("qednet://"))
    {
        uri.replace(0, 12, "qednet:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Qednet.lnk";
}
...
{
    return GetAutostartDir() / "qednet.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Qednet-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  qednet-qt [" + tr("command-line options") + "]                     " + "\n";
